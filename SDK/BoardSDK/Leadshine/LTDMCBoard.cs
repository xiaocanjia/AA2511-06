...
namespace BoardSDK
{
    public class LTDMCBoard : BoardBase, IBoardIO, IBoardAxis
    {
        private ushort _CardID = 0;

        private bool _isConnect = false;

        private bool[] axesEnable;
        ...
        public override bool Connect(string filePath)
        {
            try
            {
                ...
                _CardID = 8;
                if (_cardCount == 0)
                    _cardCount = LTDMC.dmc_board_init_eth(_CardID, "192.168.1.50");//获取初始化的板卡数
                ushort err = 0;
                LTDMC.nmc_get_errcode(0, 2, ref err);
                if (err != 0 || _cardCount == 0)
                {
                    LTDMC.dmc_soft_reset(0);
                    LTDMC.dmc_board_close();
                    for (int i = 0; i < 15; i++)    //总线卡硬件复位耗时15s左右
                        Thread.Sleep(1000);
                    LTDMC.dmc_board_init_eth(8, "192.168.1.50");         //获取初始化的板卡数
                }
                ushort cardNum = 0;
                uint[] cardTypeList = new uint[8];  //卡号固件类型
                ushort[] cardIdList = new ushort[8];//控制卡硬件ID号
                short ret = LTDMC.dmc_get_CardInfList(ref cardNum, cardTypeList, cardIdList);
                if (ret != 0)
                    return false;
                ...
                ret = LTDMC.dmc_download_memfile(_CardID, buffer, (uint)buffer.Length, fileincontrol, 201); //下载总线配置文件INI到控制卡
                if (ret != 0)
                    return false;
                uint axisCount = 0;
                ret = LTDMC.nmc_get_total_axes(_CardID, ref axisCount);
                axesEnable = new bool[axisCount];
                for (int i = 0; i < axisCount; i++)
                {
                    SetAxisServoEnabled(i, true);
                    axesEnable[i] = true;
                }
                LTDMC.nmc_clear_errcode(_CardID, 2); //清除错误码,连接后清除一下
                _isConnect = true;
            }
            catch (Exception ex)
            {
                _isConnect = false;
                LoggingIF.Log($"固高板卡连接失败：{ex.Message}");
            }
            return _isConnect;
        }
        ...
        public override bool Disconnect()
        {
            try
            {
                short error = LTDMC.dmc_board_close();
                _isConnect = false;
                return error == 0;
            }
            catch
        ...
        public override bool CheckConnect()
        {
            ...
            try
            {
                LTDMC.nmc_get_errcode(_CardID, 2, ref status);
            }
            catch
            {
                status = 1;
        ...
        public bool GoHome(int axis, double homeVelL, double homeVelH, double homeAcc, double homeDcc, uint homeMode, uint homeDir)
        {
            ...
            Thread.Sleep(500);
            error |= LTDMC.nmc_set_home_profile(_CardID, (ushort)axis, (ushort)homeMode, homeVelL, homeVelH, Tacc, Tdec, 0);//设置回原参数
            error |= LTDMC.nmc_home_move(_CardID, (ushort)axis);//启动回零
            return error == 0;
        }
        ...
        public bool ClearAlarm(int axis)
        {
            if(!_isConnect)
                return false;
            short error = LTDMC.nmc_clear_axis_errcode(_CardID, (ushort)axis);
            return error == 0;
        }
        ...
        public double GetActPos(int axis)
        {
            try
            {
                ...
                short error = 0;
                double ActPos = 0;
                error |= LTDMC.dmc_get_encoder_unit(_CardID, (ushort)axis, ref ActPos);
                return ActPos;
            }
            catch
            {
        ...
        public double GetCmdPos(int axis)
        {
            try
            {
                ...
                short error = 0;
                double ActPos = 0;
                error |= LTDMC.dmc_get_position_unit(_CardID, (ushort)axis, ref ActPos);
                return ActPos;
            }
            catch
            {
        ...
        public bool CheckIsStop(int axis)
        {
            if (!_isConnect)
                return false;
            short ret = LTDMC.dmc_check_done(_CardID, (ushort)axis);
            return ret == 1;    //0：指定轴正在运行，1：指定轴已停止
        }
        ...
        public byte GetAxisState(int axis)
        {
            try
            {
                if (!_isConnect)
                    return 0;
                uint state = LTDMC.dmc_axis_io_status(_CardID, (ushort)axis);
                bool[] bitsState = GetBits(state);
                bool[] bitsRet = new bool[8] { false, false, false, false, false, false, false, false };
                bitsRet[0] = bitsState[0];      //ALM
                bitsRet[1] = bitsState[1];      //+EL
                bitsRet[2] = bitsState[2];      //-EL
                bitsRet[3] = bitsState[4];      //ORG
                bitsRet[4] = bitsState[3];      //EMG
                bitsRet[5] = axesEnable[axis];  //使能
                return GetByte(bitsRet);
            }
            catch
            {
                return 0;
        ...
        public bool GetIn(int axisIdx, int IOIdx)
        {
            if (!_isConnect)
                return false;
            short res = LTDMC.dmc_read_inbit(_CardID, (ushort)(IOIdx));
        ...
        public bool GetOut(int axisIdx, int IOIdx)
        {
            if (!_isConnect)
                return false;
            short res = LTDMC.dmc_read_outbit(_CardID, (ushort)(IOIdx));
            return res == 0 ? true : false;
        }
        ...
        public bool SetOut(int axisIdx, int IOIdx, bool Value)
        {
            if (!_isConnect)
                return false;
            short res = LTDMC.dmc_write_outbit(_CardID, (ushort)(IOIdx), Value ? (ushort)0 : (ushort)1);
            return true;
        }
        ...
        public bool InstancyStop(int axis)
        {
            if (!_isConnect)
                return false;
            short error = LTDMC.dmc_stop(_CardID, (ushort)axis, 1);
            return error == 0;
        }
        ...
        public bool JogMove(int axis, bool isPositive)
        {
            ...
            short error = LTDMC.dmc_vmove(_CardID, (ushort)axis, (ushort)(isPositive ? 1 : 0));
            return error == 0;
        }
        ...
        {
            if (!_isConnect) return false;
            short error = LTDMC.dmc_pmove_unit(_CardID, (ushort)axis, pos, 1);  //定长运动 运动模式0：相对坐标模式，1：绝对坐标模式
            return error == 0;
        }
        ...
        {
            if (!_isConnect) return false;
            short error = LTDMC.dmc_pmove_unit(_CardID, (ushort)axis, dist, 0);  //定长运动 运动模式0：相对坐标模式，1：绝对坐标模式
            return error == 0;
        }
        ...
        public bool SetAxisServoEnabled(int axis, bool isOn)
        {
            if (!_isConnect)
                return false;
            short enable = isOn? LTDMC.nmc_set_axis_enable(_CardID, (ushort)axis): LTDMC.nmc_set_axis_disable(_CardID, (ushort)axis);//设置使能
            if(enable == 0)
            {
                ClearAlarm(axis);
                axesEnable[axis] = isOn;
                return true;
            }
            else
            {
        ...
        public bool Stop(int axis)
        {
            if (!_isConnect)
                return false;
            short error = 0;
            error |= LTDMC.dmc_set_dec_stop_time(_CardID, (ushort)axis, 0.2); //设置减速停止时间,0.3秒，整个减速时间就为0.3S,包含S段曲线
            error |= LTDMC.dmc_stop(_CardID, (ushort)axis, 0);
            return error == 0;
        }
        ...
        public void SetActPos(int axis, double pos)
        {
            ...
            LTDMC.nmc_set_offset_pos(_CardID,(ushort)axis,pos);
            LTDMC.dmc_set_position(_CardID, (ushort)axis, (int)pos);
        }
        ...
        public bool SetSpeed(int axis, double moveVelL, double moveVelH, double moveAcc, double moveDcc)
        {
            ...
            try
            {
                ...
                short error = 0;
                error |= LTDMC.dmc_set_profile_unit(_CardID, (ushort)axis, moveVelL, moveVelH, accTime, decTime, moveVelL);     //设置速度参数
                error |= LTDMC.dmc_set_s_profile(_CardID, (ushort)axis, 0, 0.01);                                               //设置S段速度参数
                return true;
            }
            catch
            {
        ...
        private bool[] GetBits(uint value)
        {
        ...
        private byte GetByte(bool[] value, int start = 0)
        {
            int length = value.Length - start;
            if (length <= 0) throw new ArgumentException("截取数组长度太小");
            if (length >= 8) length = 8;
            byte result = 0;
            for (int i = 0; i < length; i++)//遍历当前字节的每个位赋值
                result = value[i + start] ? (byte)(result | (byte)Math.Pow(2, i)) : (byte)(result & ~(byte)Math.Pow(2, i));
            return result;
        }
        ...
        public bool OscillationMove(int axis, uint count, double freq, double amp, int acqInterval)
        {
            ...
            short res = LTDMC.dmc_sine_oscillate_set_mode(_CardID, (ushort)axis, 1);
            res += LTDMC.dmc_sine_oscillate_set_cycle_num(_CardID, (ushort)axis, count);//振荡次数设置，0为无限次数振荡
            res += LTDMC.dmc_sine_oscillate_unit(_CardID, (ushort)axis, amp, freq);     //启动振荡运动
            res += LTDMC.dmc_trace_data_stop(_CardID);                                  //强制停止采样
            res += LTDMC.dmc_trace_data_reset(_CardID);                                 //复位trace采样，停止采样的时候才能调用，会清除溢出标志位和采集到的数据
            int iCycleTime = 0;
            res += LTDMC.nmc_get_cycletime(_CardID, 2, ref iCycleTime);                 //获取当前的采样周期时间，单位us（其实获取的是总线周期时间）
            res += LTDMC.dmc_trace_set_config(_CardID, (short)(acqInterval * 1000 / iCycleTime), 1, 0, 0, 0, 0, 0);
            res += LTDMC.dmc_trace_reset_config_object(_CardID);
            ushort usSlaveAddr = 0;                                                     //驱动器轴对应的的ID号，如1001,1002
            ushort usSubSlaveAddr = 0;
            res += LTDMC.nmc_get_axis_node_address(_CardID, (ushort)axis, ref usSlaveAddr, ref usSubSlaveAddr);//获取轴号对应的从站ID
            res += LTDMC.dmc_trace_add_config_object(_CardID, 19, 0x6064, 0, (short)usSlaveAddr, 4);
            res += LTDMC.dmc_trace_data_start(_CardID);                                 //启动采样
            return true;
        }

        public bool EndOscillation(int axis, out int[] dataArr)
        {
            LTDMC.dmc_sine_oscillate_stop(_CardID, (ushort)axis);
            short res = LTDMC.dmc_trace_data_stop(_CardID); //停止采样
            int iValidNum = 0;                              //已采集但未被读取的数据个数(单位：添加的数据组)
            int iFreeNum = 0;                               //剩余可用于保存采集数据的个数(单位：添加的数据组)
            int iObjectTotalBytes = 0;                      //采集对象总字节数
            int iValidObjectTotalNum = 0;                   //采集对象总个数
            res += LTDMC.dmc_trace_get_state(_CardID, ref iValidNum, ref iFreeNum, ref iObjectTotalBytes, ref iValidObjectTotalNum);
            dataArr = new int[iValidNum];
            for (int i = 0; i < iValidNum; i++)
            {
                byte[] data = new byte[iObjectTotalBytes]; //数据缓冲区
                int iByteSize = 0; //读取数据的字节数
                res = LTDMC.dmc_trace_get_data(_CardID, iObjectTotalBytes, data, ref iByteSize);
                dataArr[i] = BitConverter.ToUInt16(data, 4);
            }
            return true;
        }
